...
config = UpholderConfig(
    ...
    auto_delete_unused_indexes=False,     # Автоматическое удаление неиспользуемых индексов ⚠️
...
config = UpholderConfig(
    auto_delete_unused_indexes=True,      # ⚠️ ВКЛЮЧИТЬ автоматическое удаление
    unused_index_age_days=30,             # Удалять индексы, неиспользуемые >30 дней
    unused_index_max_size_mb=100,         # Не удалять индексы >100MB
    dry_run_mode=True                     # Сначала протестировать в dry-run режиме!
)
...
### ⚠️ Автоматическое удаление индексов
...
- ✅ **Защита PK/Unique** - никогда не удаляет первичные ключи и уникальные индексы
...
config = UpholderConfig(
    ...
    dry_run_mode=True,  # Только логирование, без реальных изменений
    unused_index_age_days=30,
    unused_index_max_size_mb=50   # Начать с меньшего лимита
)

# Шаг 2: Мониторинг логов в течение недели
# Шаг 3: Только потом включить в production с осторожностью
config = UpholderConfig(
    auto_delete_unused_indexes=True,
    dry_run_mode=False,  # ⚠️ РЕАЛЬНЫЕ ИЗМЕНЕНИЯ!
    unused_index_age_days=90,     # Дольше ждать в production
    unused_index_max_size_mb=10   # Меньше рисковать
)
```
...
✅ Automatically deleted 1 unused indexes
```
