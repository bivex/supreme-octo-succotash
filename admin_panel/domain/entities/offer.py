...
class Offer:
    ...
    def _validate_currency_consistency(self) -> None:
        ...
        if self.cost_per_click and self.cost_per_click.currency != base_currency:
            raise ValidationError("Cost per click and payout must use the same currency")

        if self.revenue.currency != base_currency:
            raise ValidationError("Revenue and payout must use the same currency")

        if self.cost.currency != base_currency:
            raise ValidationError("Cost and payout must use the same currency")
